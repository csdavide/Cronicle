...
module.exports = Class.create({
	...
	launchJob: function(event, callback) {
		...
		function(err) {
			...
			for (var idx = 0, len = servers.length; idx < len; idx++) {
				...
				if (!job.notify_template && category.notify_template) job.notify_template = category.notify_template;
	...
	finishJob: function(job) {
		...
		var to = '';
                var sendMail = false;
  var email_template = job.notify_template ? job.notify_template : "conf/emails"
 // self.logActivity( 'warning', { description: (job.notify_success!='') } );
 // self.logActivity( 'warning', { description: (job.notify_fail!='') } );
		if (job.notify_success && job.notify_success != '' && (job.code == 0)) {
			email_template = email_template + "/job_success.txt"
			to = job.notify_success;
                        sendMail = true;
		}
		else if (job.notify_fail && job.notify_fail != '' && (job.code != 0)) {
			email_template = email_template + "/job_fail.txt";
			to = job.notify_fail;
                        sendMail = true;
		}
